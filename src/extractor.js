...
export async function extractBusinessData(page) {
  try {
    ...
    try {
      ...
      const reviewsButton = await page.$('button[aria-label*="reviews"], button[data-tooltip*="reviews"]');
      if (reviewsButton) {
        await reviewsButton.click().catch(() => {});
        await sleep(1000);
      }

      const amenitiesButton = await page.$('button[aria-label*="Amenities"], button[aria-label*="Highlights"]');
      if (amenitiesButton) {
        await amenitiesButton.click().catch(() => {});
        await sleep(1000);
      }
    } catch (error) {
...
export async function extractBusinessUrls(page, maxResults = 100) {
  ...
  try {
    // Wait for feed to load with multiple selector options
    const feedSelectors = [
      'div[role="feed"]',
      'div[role="main"]',
      'div.PbZDve',
      'div[data-listview-type="default"]',
      'div[class*="feed"]'
    ];
    ...
    for (const selector of feedSelectors) {
      try {
        feedElement = await page.waitForSelector(selector, { timeout: 3000 }).catch(() => null);
        if (feedElement) {
          break;
        }
      } catch (e) {
    ...
    // Extract URLs even without feed selector
    ...
    initialUrls.forEach(url => urls.add(url));
    ...
    console.error('Error extracting business URLs:', error.message);
