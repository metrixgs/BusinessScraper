...
app.post('/api/scrape', async (req, res) => {
  const { searchType, query, location, zipCode, latitude, longitude, radiusMeters, maxResults = 50 } = req.body;
  ...
  (async () => {
    try {
      ...
      if (searchType === 'location') {
        logger.info(`Searching in location: ${location}`);
        results = await scraper.search({ query, location, maxResults });
      } else if (searchType === 'zipcode') {
        logger.info(`Searching in ZIP code: ${zipCode}`);
        results = await scraper.searchByZipCode({ query, zipCode, maxResults });
...
app.listen(PORT, () => {
  ...
  console.log(`\n[${timestamp}] [INFO] Google Maps Scraper Web Interface`);
  console.log(`[${timestamp}] [INFO] Server running on http://localhost:${PORT}\n`);
});
